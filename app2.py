...
# Load the certificate template from the 'assets' directory
def load_template():
    template_path = "assets/c_iot-1.png"
    if os.path.exists(template_path):
        return Image.open(template_path)
    else:
        st.error("Certificate template not found in the 'assets' directory.")
        st.stop()
...
horizontal_position = st.slider("Horizontal position (%)", min_value=0, max_value=100, value=43, step=1)  # Default horizontal position set to 43%
vertical_position = st.slider("Vertical position (%)", min_value=0, max_value=100, value=50, step=1)  # Default vertical position set to 50%
        ...
        if name:
            ...
            if font_file:
                try:
                    ...
                    st.error("Unable to open the uploaded font file.")
                    return []
            else:
                # Check if default font file exists
                font_path = "assets/arial.ttf"
                if os.path.exists(font_path):
                    try:
                        font = ImageFont.truetype(font_path, font_size)
                    except OSError:
                        st.error("Unable to load the default font file.")
                        return []
                else:
                    st.error("Default font file 'arial.ttf' not found.")
                    return []
